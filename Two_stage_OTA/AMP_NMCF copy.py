...
class AMPNMCFEnv(gym.Env, CktGraph, DeviceParams):
    ...
    def __init__(self, **kwargs):
        ...
        self.NSGA_agent = NSGA_Agent(population_size=16) #tim.tan
        ...
        self.initial_NSGA_population(self.NSGA_agent.population, 2)
    ...
    def initial_NSGA_population(self, population, times):
        for i in range(times):
            ...
            print(results)
            selected_individuals = sel_nsga_iii(population, k=8)
            if i == times-1:
    ...
    def _do_simulation(self, action: np.array):
        ...
        Ib = 10.0              # 10 uA (¶ÔÓ¦Íø±íÖÐµÄ {Ib}e-6)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.parameters_population = np.random.uniform(
        ...
        with mp.Pool(processes=mp.cpu_count()) as pool:
            results = pool.map(rst_simulate_individual, [(self, ind, j) for j, ind in enumerate(self.parameters_population)])
    ...
    '''def reset(self, seed=None, options=None):
        ...
        return observation_batch, {}'''#tim.tan
    ...
    def _get_obs(self, op_results, Ib):
        ...
        #f = open(f'{SPICE_NETLIST_DIR}/op_results.txt','w')
        #f.write(op_results)
        #f.close()
...
def rst_simulate_individual(args):
    ...
    for _ in range(max_retries):
        try:
            ...
            # Èç¹û·ÂÕæÊ§°Ü£º
            # 1. ÖØÐÂËæ»úÉú³ÉÒ»×é²ÎÊý (Resample)
            current_ind = np.random.uniform(env.action_space_low, env.action_space_high)
            
            # 2. ±ðÍüÁËÈ¡ÕûÔ¼Êø (Integer Constraints)
            idx_integer_params = [8, 12]
            for i in idx_integer_params:
                current_ind[i] = np.round(current_ind[i])
                
            # continue ¼ÌÐøÏÂÒ»´ÎÑ­»·³¢ÊÔ
            continue
            
    # Èç¹ûÖØÊÔÁË 20 ´Î»¹ÊÇÈ«¹ÒÁË£¨ËµÃ÷²ÎÊý¿Õ¼äÌ«¶ñÁÓ£©£¬Ö»ÄÜ·µ»ØÊ§°Ü×´Ì¬
    # ÕâÀïÄãÐèÒª¹¹ÔìÒ»¸ö·ûºÏ Shape µÄÈ« 0 Obs
    return observation, {
                'TC': [0,-0.999],
                'Power': [0,-0.999],
                'vos': [0,-0.999],
...
#     return observation, info, reward, done \tim.tan
