...
from test_nsgaiii import AMPNMCF
...
def evaluate(individual):
    amp = AMPNMCF()
    amp.do_simulation(np.array(individual))

if __name__ == "__main__":
    CktGraph = GraphAMPNMCF
    results = OutputParser2(CktGraph())
    op_results = results.dcop('AMP_NMCF_op')
    print(results,op_results)
    creator.create("FitnessMin", base.Fitness, weights=(-1.0, -1.0, 1.0))
    creator.create("Individual", list, fitness=creator.FitnessMin)

    individuals = [creator.Individual([random.random(), random.random(), random.random()]) for _ in range(50)]
    for ind in individuals:
        ind.fitness.values = (random.random(), random.random(), random.random())

    selected_individuals = sel_nsga_iii(individuals, k=20) 

    print(f"Selected {len(selected_individuals)} individuals.",selected_individuals)