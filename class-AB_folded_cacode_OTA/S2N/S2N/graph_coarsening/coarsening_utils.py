...
def coarsen(
    ...
    """
    This function provides a common interface for coarsening_pyg algorithms that contract subgraphs
    ...
    for level in tqdm(range(1, max_levels + 1), desc="coarsening_utils.coarsen by levels"):
...
################################################################################
# General coarsening_pyg utility functions
################################################################################
...
def get_coarsening_matrix(G, partitioning):
    """
    This function should be called in order to build the coarsening_pyg matrix C.
    ...
    Returns
    -------
    C : the new coarsening_pyg matrix
...
def coarsening_quality(G, C, kmax=30, Uk=None, lk=None):
    """
    Measures how good is a coarsening_pyg.
    ...
    -------
    metric : dictionary
        Contains all relevant metrics for coarsening_pyg quality:
...
def plot_coarsening(
    ...
    """
    Plot a (hierarchical) coarsening_pyg
...
def contract_variation_linear(G, A=None, K=10, r=0.5, mode="neighborhood"):
    ...
    # The following is correct only for a single level of coarsening_pyg.
    ...
    while len(family) > 0:
        ...
        if not any(i_marked):
            ...
            # all vertices are unmarked: add i_set to the coarsening_pyg list
            marked[i_set] = True
