...
class AMPNMCFEnv(gym.Env, CktGraph, DeviceParams):
    ...
    def __init__(self, **kwargs):
        ...
        self.NSGA_agent = NSGA_Agent(200)
        ...
        self.initial_NSGA_population(self.NSGA_agent.population, 1100)
    ...
    def initial_NSGA_population(self, population, times, file_path="nsga_pure_results.csv"):
        """
        :param population: ³õÊ¼ÖÖÈº
        :param times: µü´ú´ÎÊý
        :param file_path: ¼ÇÂ¼½á¹ûµÄ CSV ÎÄ¼þÂ·¾¶
        """
        for i in range(times):
            #print(f"NSGA Generation/Step: {i}")
            
            # ¶à½ø³ÌÆÀ¹ÀÖÖÈº
            with mp.Pool(processes=mp.cpu_count()) as pool:
                results = pool.map(simulate_individual, [(self, ind, j) for j, ind in enumerate(population)])

            # ´ò¿ªÎÄ¼þ×¼±¸×·¼ÓÐ´Èëµ±Ç°´úµÄÊý¾Ý
            with open(file_path, mode='a', newline='') as file:
                writer = csv.writer(file)
                
                for ind, info in zip(population, results):
                    # 1. Îª¸öÌå¸³ÓèÊÊÓ¦¶ÈÖµ (Ê¹ÓÃ±ê×¼»¯/´¦ÀíºóµÄÖµ: Ë÷Òý [1])
                    ind.fitness.values = (
                        info['Power'][1], info['dcgain'][1], info['GBW'][1], 
                        info['phase_margin (deg)'][1], info['TC'][1], info['vos'][1], 
                        info['cmrrdc'][1], info['PSRP'][1], info['PSRN'][1], 
                        info['sr'][1], info['settlingTime'][1], info['reward']
                    )
                    
                    # 2. ½«¸öÌåµÄÔ­Ê¼ÐÔÄÜ²ÎÊýÐ´Èë CSV ÎÄ¼þ (Ê¹ÓÃÔ­Ê¼Öµ: Ë÷Òý [0])
                    # ²Î¿¼ main() º¯Êý£¬µÚÒ»ÁÐ·Å reward£¬ºóÃæ½Ó¸÷ÏîÐÔÄÜµÄÔ­Ê¼Öµ£¬×îºóÒ»ÁÐÔÙ·ÅÒ»±é reward (¸ù¾ÝÄãµÄÔ­´úÂë½á¹¹)
                    writer.writerow([
                        info['reward'],  # Ìæ´ú RL ÖÐµÄ r[i]
                        info['Power'][0],
                        info['dcgain'][0],
                        info['GBW'][0],
                        info['phase_margin (deg)'][0],
                        info['TC'][0],
                        info['vos'][0],
                        info['cmrrdc'][0],
                        info['PSRP'][0],
            ...
            # NSGA-III Ñ¡Ôñ²Ù×÷
            selected_individuals = sel_nsga_iii(population, k=100)
            
            # Èç¹ûÊÇ×îºóÒ»´Îµü´ú£¬ÔòÌáÇ°Ìø³ö£¬²»ÔÙÉú³É×Ó´ú
            if i == times - 1:
                break
...
class BoPpoAMPNMCFEnv(gym.Env, CktGraph, DeviceParams):
    def __init__(self, optimal_x0: np.array = None, **kwargs):
        gym.Env.__init__(self)
        CktGraph.__init__(self)
        DeviceParams.__init__(self, self.ckt_hierarchy)
        self.CktGraph = GraphAMPNMCF()
        # The BO-optimized starting point. If None, fallback to a random valid point.
        self.optimal_x0 = optimal_x0 
        
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=self.obs_shape, dtype=np.float32 )
        
        # --- ROBUST-OPT DISCRETE ACTION SPACE ---
        # 3 discrete actions per parameter: 0 (decrease), 1 (keep), 2 (increase)
        num_params = len(self.action_space_low)
        self.action_space = spaces.MultiDiscrete([3] * num_params)
        ...
        self.current_step = 0
        self.max_steps_per_episode = 500 # Prevent infinite wandering

    def reset(self, seed=None, options=None):
        self.current_step = 0
        
        if self.optimal_x0 is not None:
            noise = np.random.normal(0, self.delta_action * 1)
            self.current_params = np.clip(
                self.optimal_x0.copy() + noise,
                self.action_space_low, self.action_space_high
            )
        else:
            # Fallback if BO hasn't run yet
            self.current_params = np.random.uniform(self.action_space_low, self.action_space_high)
            
        # Ensure integer parameters are rounded
        idx_integer_params = [2, 5, 8, 11, 14, 17, 20, 22, 23]
        for idx in idx_integer_params:
            self.current_params[idx] = np.round(self.current_params[idx])

        # Run initial simulation to get starting observation
        observation, info = self._run_sim_and_get_obs(self.current_params)
        return observation, info
    ...
    def step(self, action):
        ...
        # ¶¯×÷½âÂë
        step_multiplier = action - 1 
        delta = self.delta_action * step_multiplier
        
        # ¸üÐÂ²ÎÊý²¢ÏÞÖÆÔÚºÏ·¨±ß½çÄÚ
        self.current_params = np.clip(
        ...
        reward = info.get('reward', -11.0)
        spice_failed = info.get('spice_failed', False)
        
        # ==========================================
        # ºËÐÄÖÕÖ¹Ìõ¼þÂß¼­¸ÄÔì (RoSE-Opt Termination)
        # ==========================================
        if reward >= 0:
            # Ìõ¼þ 1£ºËùÓÐÖ¸±ê´ï±ê£¬³É¹¦Íê³ÉÉè¼Æ
            reward = 1.0 # ¹éÒ»»¯³É¹¦½±Àø
            terminated = True
        elif spice_failed:
            # Ìõ¼þ 2£º·ÂÕæ±ÀÀ£ËÀÇø£¬±ØÐëÁ¢¿Ì¿³¶Ï Episode
            reward = -11.0 
            terminated = True 
        else:
            # Ìõ¼þ 3£ºÕý³£·ÂÕæ£¬¼ÌÐøÌ½Ë÷
            terminated = False
    ...
    def _run_sim_and_get_obs(self, params):
        """Helper to run sim with retry logic for SPICE failures."""
        max_retries = 3
        for _ in range(max_retries):
            try:
                op_results, sim_results, Ib = self.do_simulation(params)
                observation = self._get_obs(op_results, Ib)
                info = self._get_info(sim_results)
                info['spice_failed'] = False  # Ã÷È·±ê¼Ç£º·ÂÕæ³É¹¦
                return observation, info
            except Exception as e:
                # Óöµ½ËÀÇøÊ±£¬¼ÓÒ»µãÐ¡ÔëÉù³¢ÊÔÌø³ö
                params = np.clip(
                    params + np.random.normal(0, self.delta_action * 0.5), 
                    self.action_space_low, 
                    self.action_space_high
                )
                idx_integer_params = [2, 5, 8, 11, 14, 17, 20, 22, 23]
        ...
        # Èç¹ûÁ¬ÐøÖØÊÔÒÀÈ»Ê§°Ü£¬·µ»ØÈ« 0 ×´Ì¬£¬²¢´òÉÏÖÂÃüÊ§°Ü±êÇ©
        penalty_obs = np.zeros((29, 12), dtype=np.float32)
        penalty_info = {
            'TC': [0,-1], 'Power': [0,-1], 'vos': [0,-1], 'cmrrdc': [0,-1], 
            'dcgain': [0,-1], 'GBW': [0,-1], 'phase_margin (deg)': [0,-1], 
            'PSRP': [0,-1], 'PSRN': [0,-1], 'sr': [0,-1], 'settlingTime': [0,-1], 
            'reward': -11.0,
            'spice_failed': True  # CRITICAL: Ã÷È·±ê¼Ç£º·ÂÕæ³¹µ×Ê§°Ü
        }
        return penalty_obs, penalty_info
